...
async function setJson(env: Env, key: string, value: unknown): Promise<void> {
  ...
  try {
    await env.CART_KV.put(key, JSON.stringify(value), { expirationTtl: 60 * 60 * 24 * 60 });
  } catch (err) {
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    const body = (await context.request.json()) as { items?: WishlistItem[] };
    const sessionId = getSessionId(context.request);
    const items = Array.isArray(body.items) ? body.items : [];

    await setJson(context.env, `wishlist:${sessionId}`, items);
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  const sessionId = getSessionId(context.request);
  const items = await getJson<WishlistItem[]>(context.env, `wishlist:${sessionId}`, []);
